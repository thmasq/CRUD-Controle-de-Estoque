...
use stock_application::{
    CategoryService, ProductService, StockItemService, StockTransactionService, WarehouseService,
};
use stock_infrastructure::{
    db::establish_connection_pool,
    repositories::{
        category_repository::DieselCategoryRepository, product_repository::DieselProductRepository,
        stock_item_repository::DieselStockItemRepository,
        stock_transaction_repository::DieselStockTransactionRepository,
        warehouse_repository::DieselWarehouseRepository,
    },
};
...
async fn main() -> std::io::Result<()> {
    ...
    tracing_subscriber::registry()
        .with(tracing_subscriber::EnvFilter::new(
            std::env::var("RUST_LOG")
                .unwrap_or_else(|_| "stock_web_server=debug,actix_web=info".into()),
        ))
    ...
    let transaction_service = Arc::new(StockTransactionService::new(
        transaction_repo,
        stock_item_repo,
    ));
    ...
    let server = HttpServer::new(move || {
        App::new()
            ...
            .route(
                "/categories",
                web::get().to(handlers::category::list_categories),
            )
            .route(
                "/categories/new",
                web::get().to(handlers::category::new_category_form),
            )
            .route(
                "/categories/{id}/edit",
            ...
            .route(
                "/categories",
                web::post().to(handlers::category::create_category),
            )
            .route(
                "/categories/{id}",
                web::put().to(handlers::category::update_category),
            )
            .route(
                "/categories/{id}",
            ...
            .route(
                "/products/new",
                web::get().to(handlers::product::new_product_form),
            )
            .route(
                "/products/{id}/edit",
            ...
            .route(
                "/products",
                web::post().to(handlers::product::create_product),
            )
            .route(
                "/products/{id}",
                web::put().to(handlers::product::update_product),
            )
            .route(
                "/products/{id}",
                web::delete().to(handlers::product::delete_product),
            )
            // Warehouses
            .route(
                "/warehouses",
                web::get().to(handlers::warehouse::list_warehouses),
            )
            .route(
                "/warehouses/new",
            ...
            .route(
                "/warehouses",
                web::post().to(handlers::warehouse::create_warehouse),
            )
            .route(
                "/warehouses/{id}",
                web::put().to(handlers::warehouse::update_warehouse),
            )
            .route(
                "/warehouses/{id}",
            ...
            .route(
                "/stock-items",
                web::get().to(handlers::stock_item::list_stock_items),
            )
            .route(
                "/stock-items/new",
            ...
            .route(
                "/stock-items",
                web::post().to(handlers::stock_item::create_stock_item),
            )
            .route(
                "/stock-items/{id}",
