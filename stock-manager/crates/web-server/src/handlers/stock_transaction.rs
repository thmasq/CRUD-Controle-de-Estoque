...
use crate::AppState;
use crate::dtos::stock_transaction::{
    TransactionDto, TransactionFilterRequest, TransactionListTemplate,
};
...
pub async fn list_transactions(
    ...
    let mut transactions = transaction_service
    ...
    let stock_item_map: std::collections::HashMap<_, _> =
        stock_items.iter().map(|si| (si.id, si.clone())).collect();
