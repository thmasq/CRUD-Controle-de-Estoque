...
use crate::dtos::stock_item::{
    StockItemCreateRequest, StockItemDto, StockItemFilterRequest, StockItemFormTemplate,
    StockItemListTemplate, StockItemTransactionTemplate, StockItemUpdateRequest,
};
...
pub async fn list_stock_items(
    ...
    // Get all data
    let mut stock_items = stock_service
        .get_all_stock_items()
        .await
        .unwrap_or_default();
    ...
    // Create lookup maps
    let product_map: std::collections::HashMap<_, _> =
        products.iter().map(|p| (p.id, p.clone())).collect();

    let warehouse_map: std::collections::HashMap<_, _> =
        warehouses.iter().map(|w| (w.id, w.clone())).collect();
...
pub async fn edit_stock_item_form(
    path: web::Path<Uuid>,
    state: web::Data<AppState>,
) -> Result<HttpResponse> {
    ...
    if let Some(stock_item) = stock_service
    ...
    match stock_service.create_stock_item(dto).await {
        ...
        Err(e) => {
            Ok(HttpResponse::InternalServerError()
                .body(format!("Failed to create stock item: {}", e)))
        }
    }
}
...
pub async fn update_stock_item(
    ...
    match stock_service.update_stock_item(dto).await {
        ...
        Err(e) => {
            Ok(HttpResponse::InternalServerError()
                .body(format!("Failed to update stock item: {}", e)))
        }
    }
}
...
pub async fn delete_stock_item(
    ...
    match stock_service.delete_stock_item(stock_item_id).await {
        ...
        Err(e) => {
            Ok(HttpResponse::InternalServerError()
                .body(format!("Failed to delete stock item: {}", e)))
        }
    }
}
...
pub async fn create_transaction(
    ...
    match transaction_service.create_transaction(dto).await {
        ...
        Err(e) => Ok(HttpResponse::InternalServerError()
            .body(format!("Failed to create transaction: {}", e))),
    }
}
