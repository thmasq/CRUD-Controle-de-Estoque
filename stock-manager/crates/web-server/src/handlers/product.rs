...
use crate::dtos::product::{
    ProductCreateRequest, ProductDto, ProductFilterRequest, ProductFormTemplate,
    ProductListTemplate, ProductUpdateRequest,
};
...
pub async fn edit_product_form(
    ...
    if let Some(product) = product_service
...
pub async fn create_product(
    state: web::Data<AppState>,
    form: web::Form<ProductCreateRequest>,
) -> Result<HttpResponse> {
    ...
    match service.create_product(dto).await {
        Ok(product) => {
            ...
            let status_text = if product_dto.is_active {
                "Active"
            } else {
                "Inactive"
            };
...
pub async fn delete_product(
    ...
    match service.delete_product(product_id).await {
        ...
        Err(e) => Ok(
            HttpResponse::InternalServerError().body(format!("Failed to delete product: {}", e))
        ),
    }
}
