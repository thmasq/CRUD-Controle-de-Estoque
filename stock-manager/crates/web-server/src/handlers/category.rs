...
use crate::dtos::category::{
	CategoryCreateRequest, CategoryDto, CategoryFormTemplate, CategoryListTemplate, CategoryUpdateRequest,
};
...
pub async fn edit_category_form(path: web::Path<Uuid>, state: web::Data<AppState>) -> Result<HttpResponse> {
...
pub async fn delete_category(path: web::Path<Uuid>, state: web::Data<AppState>) -> Result<HttpResponse> {
	let category_id = path.into_inner();
	let service = state.category_service.clone();
