...
impl StockTransactionService {
    ...
    pub async fn get_transactions_by_stock_item(
        &self,
        stock_item_id: Uuid,
    ) -> anyhow::Result<Vec<StockTransaction>> {
        self.transaction_repository
            .find_by_stock_item(stock_item_id)
            .await
    }

    pub async fn create_transaction(
        &self,
        dto: StockTransactionCreateDto,
    ) -> anyhow::Result<StockTransaction> {
        // Find the stock item
        let stock_item = self
            .stock_item_repository
